...
library OrderStoreUtils {
    ...
    bytes32 public constant CANCELLATION_RECEIVER =
        keccak256(abi.encode("CANCELLATION_RECEIVER"));
    ...
    bytes32 public constant VALID_FROM_TIME =
        keccak256(abi.encode("VALID_FROM_TIME"));
    bytes32 public constant UPDATED_AT_TIME =
        keccak256(abi.encode("UPDATED_AT_TIME"));
    ...
    function get(
        ...
        order.setValidFromTime(
            dataStore.getUint(keccak256(abi.encode(key, VALID_FROM_TIME)))
        );

        order.setUpdatedAtTime(
            dataStore.getUint(keccak256(abi.encode(key, UPDATED_AT_TIME)))
        );

        order.setIsLong(dataStore.getBool(keccak256(abi.encode(key, IS_LONG))));
        order.setAutoCancel(
            dataStore.getBool(keccak256(abi.encode(key, AUTO_CANCEL)))
        );
