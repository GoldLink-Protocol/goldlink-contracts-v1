...
abstract contract MockAccountGmxHelpers is MockAccountSetup {
    ...
    ) internal returns (IGmxV2OrderTypes.CreateOrderParams memory order) {
    ...
    function _executeGmxOrder(bytes32 orderKey) internal {
        ...
        params.realtimeFeedData[0] = abi.encode(
            _createRealTimeFeedReport(market.shortToken, priceShort)
        );
        params.realtimeFeedData[1] = abi.encode(
            _createRealTimeFeedReport(market.longToken, priceLong)
        );
        vm.prank(ORACLE_SIGNER);
        IOrderHandler(0x352f684ab9e97a6321a13CF03A61316B681D9fD2).executeOrder(
            orderKey,
            params
        );
    }
    ...
    function _getOrderInfo(
        bytes32 orderKey
    ) internal returns (IGmxV2OrderTypes.Props memory) {
        return
            GmxFrfStrategyMetadata.GMX_V2_READER.getOrder(
                GmxFrfStrategyMetadata.GMX_V2_DATASTORE,
                orderKey
            );
    }
    ...
    function _getMarket(
        address market
    ) internal returns (IGmxV2MarketTypes.Props memory) {
        return
            GmxFrfStrategyMetadata.GMX_V2_READER.getMarket(
                GmxFrfStrategyMetadata.GMX_V2_DATASTORE,
                market
            );
    }

    function _createRealTimeFeedReport(
        address asset,
        uint256 assetPrice
    ) internal returns (IOrderHandler.RealtimeFeedReport memory report) {
        report.feedId = GmxFrfStrategyMetadata.GMX_V2_DATASTORE.getBytes32(
            realtimeFeedIdKey(asset)
        );
        ...
        report.upperBlockhash = ARBSYS.arbBlockHash(
    ...
    function realtimeFeedIdKey(address token) internal pure returns (bytes32) {
        return
            keccak256(
                abi.encode(keccak256(abi.encode("REALTIME_FEED_ID")), token)
            );
    }
}
