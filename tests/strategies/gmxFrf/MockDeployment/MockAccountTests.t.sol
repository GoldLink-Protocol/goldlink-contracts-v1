...
contract MockAccountTests is MockAccountHelpers {
    ...
    function testLiquidateAssets() public {
        ...
        SwapHandler.SwapData memory dat = SwapHandler.SwapData(SwapHandler.SwapType.Liquidation, address(ACCOUNT), address(this), WETH_USDC_UNIV3, IERC20(WETH), true, 3e16, 484016920170066100000);
        ACCOUNT.executeLiquidateAssets(address(WETH), 3e16, address(SWAPHANDLER), address(SWAPHANDLER), abi.encode(dat));
